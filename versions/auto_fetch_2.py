...
def process_single_job(url: str, base_cv_text: str, target_title: str, provider: str, work_model: str, job_type: str, visa: str) -> bool:
    """
    Helper function to run a single job evaluation in an isolated thread.
    Returns True if successfully processed and added to DB, False otherwise.
    """
...
def run_auto_fetch_pipeline(base_cv_text: str, target_title: str, location: str, provider: str = "gemini", max_jobs: int = 5, job_board: str = "All Supported Boards", work_model: str = "Any", job_type: str = "Any", visa: str = "Any"):
    ...
    Now uses ThreadPoolExecutor to run AI agent requests in parallel.
    """
    ...
    # Identify which URLs are completely new
    new_urls = []
    ...
    if new_urls:
        # Launching Parallel Threads to slash processing time
        print(f"⚡ Launching parallel processing for {len(new_urls)} jobs...", flush=True)
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(
                    process_single_job, url, base_cv_text, target_title, provider, work_model, job_type, visa
                ) for url in new_urls
            ]
            
            # Wait for them to finish and safely tally the score
