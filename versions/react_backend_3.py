...
# NEW PHASE 5 SCHEMA: Cold Email and Portfolio
...
# NEW PHASE 5 NODE: Outreach & Portfolio Generation
def outreach_node(state: AgentState):
    ...
    system_prompt = (
        ...
        "1. Write a highly tailored, concise cold email addressed to the Hiring Manager or Recruiter. "
        "It MUST reference specific pain points found in the job description and highlight exactly how the candidate's CV solves them. "
        "2. Generate a targeted Mini-Portfolio strategy. Suggest 1 or 2 specific projects (either extracting from the user's CV or suggesting a quick weekend project) "
        "that perfectly align with the target role's tech stack and domain."
    )
    ...
    try:
        ...
        # Graceful fallback
        result = OutreachOutput(
            cold_email="Subject: Application for {role}\n\nHi Team,\n\nI am reaching out regarding the open position. My background aligns well with your requirements, particularly in my recent projects.\n\nBest,\nCandidate",
            portfolio_strategy="- Highlight a recent relevant project.\n- Create a short GitHub repo demonstrating key skills required in the JD."
        )
...
def build_job_agent():
    ...
    builder.add_node("outreach", outreach_node) # Phase 5 addition
    ...
    builder.add_edge("coach", "outreach")       # Phase 5 addition
