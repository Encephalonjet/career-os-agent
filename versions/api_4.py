...
# ****************** NEW: CV PROFILE ENDPOINTS *********************


@app.get("/api/profile/cv")
def get_profile_cv():
    """Allows the React UI to pull the saved CV instantly on startup."""
    return {"cv_text": database.get_user_cv()}


# ***************** JOB EVALUATION & SEARCH ENDPOINTS *********************
...
def auto_fetch_endpoint(
    ...
    try:
        ...
        # NEW: Automatically save the latest CV text securely to the database
        database.save_user_cv(extracted_cv_text)
            
        added_count = run_auto_fetch_pipeline(
        ...
        return {"status": "success", "added_jobs": added_count}
